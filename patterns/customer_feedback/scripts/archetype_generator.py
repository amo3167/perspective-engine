...
def generate_personas(
    total: int, profiles: dict[str, Any] | None = None, seed: int | None = None
) -> list[dict[str, Any]]:
    ...
    for arch, count in allocated:
        ...
        for _ in range(count):
            ...
            patience = random.randint(
                arch["patience_range"][0], arch["patience_range"][1]
            )
            ...
            if demographics:
                if "regions" in demographics:
                    persona["region"] = _pick_demographic(demographics["regions"])[
                        "label"
                    ]
                if "usage_tenure" in demographics:
                    persona["tenure"] = _pick_demographic(demographics["usage_tenure"])[
                        "label"
                    ]
                if "plan_tier" in demographics:
                    persona["plan"] = _pick_demographic(demographics["plan_tier"])[
                        "label"
                    ]
                if "use_case" in demographics:
                    persona["use_case"] = _pick_demographic(demographics["use_case"])[
                        "label"
                    ]
                if "emotional_state" in demographics:
                    persona["emotional_state"] = _pick_demographic(
                        demographics["emotional_state"]
                    )["label"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate synthetic customer personas from archetypes."
    )
    parser.add_argument(
        "--count", type=int, default=10, help="Total number of personas to generate"
    )
    parser.add_argument(
