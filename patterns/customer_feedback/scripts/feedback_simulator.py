...
try:
    ...
    try:
        ...
        from agent_gateway.config import (
            LITELLM_API_BASE as _BASE,
            LITELLM_API_KEY as _KEY,
        )
...
DRY_RUN_FEEDBACK: dict[str, dict[str, Any]] = {
    "power_user": {
        ...
        "concerns_pool": [
            "30-second update latency too slow",
    ...
    "casual_user": {
        ...
        "concerns_pool": [
            "might be confusing",
            "hope it doesn't slow down the app",
            "too many features I won't use",
        ],
        "requests_pool": [
            "simple mode/view",
            "push notifications for big changes",
    ...
    "privacy_skeptic": {
        ...
        "requests_pool": [
            "data export with full deletion option",
            "privacy policy specific to dashboard",
    ...
    "change_resistant": {
        ...
        "concerns_pool": [
            "replacing a working page",
            "unnecessary complexity",
            "learning curve",
            "things moved around again",
        ],
        "requests_pool": [
            "keep the old simple view as an option",
    ...
    "budget_conscious": {
        ...
        "concerns_pool": [
            "hidden costs",
            "feature gating behind premium",
            "competitors offer similar for free",
        ],
        "requests_pool": [
            "keep core dashboard on free tier",
    ...
    "enterprise_admin": {
        ...
        "requests_pool": [
            "RBAC for dashboard widgets",
            "bulk export for compliance",
            "admin-controlled default views",
        ],
    },
    "newcomer": {
        "sentiment_range": (3, 4),
    ...
    "data_driven": {
        ...
        "requests_pool": [
            ...
            "JSON/Parquet export",
            "webhook for real-time data push",
        ],
    },
    "social_sharer": {
        "sentiment_range": (4, 5),
        ...
        "concerns_pool": [
            "screenshots might expose private data",
            "no social sharing built in",
        ],
        "requests_pool": [
            "one-click share with privacy filter",
            "OG preview cards for links",
            "achievement badges",
        ],
    },
}
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                ...
                wait = 5 * (attempt + 1)
                logger.warning(
                    f"[{persona['persona_id']}] Rate limited (429). Waiting {wait}s..."
                )
                await asyncio.sleep(wait)
            else:
                logger.error(
                    f"[{persona['persona_id']}] Gemini error {resp.status_code}: {resp.text[:200]}"
                )
        except Exception as e:
            logger.error(
                f"[{persona['persona_id']}] Connection error (attempt {attempt + 1}): {e}"
            )
    ...
    for attempt in range(max_attempts):
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.error(
                    f"[{persona['persona_id']}] MiniMax error {resp.status_code}: {resp.text[:200]}"
                )
        except Exception as e:
            logger.error(
                f"[{persona['persona_id']}] Connection error (attempt {attempt + 1}): {e}"
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                ...
                logger.error(
                    f"[{persona['persona_id']}] LLM error {resp.status_code}: {resp.text[:200]}"
                )
        except Exception as e:
            logger.error(
                f"[{persona['persona_id']}] Connection error (attempt {attempt + 1}): {e}"
...
async def call_bedrock_converse(
    ...
    for attempt in range(4):
        try:
            ...
            logger.error(
                f"[{persona['persona_id']}] Bedrock error (attempt {attempt + 1}): {e}"
            )
            await asyncio.sleep(3)
...
async def _run_survey_pipeline(
        ...
        survey_data = {
            ...
            "questions": [
                ...
                },
                {
                    "id": "q3",
                    "type": "open",
                    "text": "What concerns do you have about this new dashboard?",
                },
            ],
        }
    ...
    if dry_run:
            ...
            for q in questions:
                if q["type"] == "rating":
                    ...
                    if q.get("allow_multiple"):
                        val = _random.sample(
                            opts, k=min(_random.randint(1, 3), len(opts))
                        )
                    else:
        ...
        (run_dir / "personas.json").write_text(
            json.dumps(_personas, indent=2), encoding="utf-8"
        )
        logger.info(
    ...
    await _broadcast(
        broadcast_url,
        {
            ...
            "step": 3,
            "agent": "customers",
            "artifact": "survey_responses.json",
            "count": count,
            "elapsed_ms": int((_time.monotonic() - _step_t0) * 1000),
        },
    )
    ...
    await _broadcast(
        broadcast_url,
        {
            "feedback_type": "pipeline_complete",
            "run_id": run_dir.name,
...
async def _run_feedback_mode(
    ...
    if dry_run:
        ...
        for batch_start in range(0, len(personas), batch_size):
            ...
            logger.info(
                f"Processing agentic batch {batch_start + 1}-{batch_end} of {len(personas)}..."
            )
        ...
        try:
            for batch_start in range(0, len(personas), batch_size):
                ...
                batch_end = min(batch_start + batch_size, len(personas))
                logger.info(
                    f"Processing batch {batch_start + 1}-{batch_end} of {len(personas)}..."
                )
    ...
    logger.info(
        f"Simulation complete: {len(all_responses)}/{len(personas)} responses collected"
    )
...
async def run_simulation(
    ...
    for p in personas:
        archetype_counts[p["archetype_label"]] = (
            archetype_counts.get(p["archetype_label"], 0) + 1
        )
...
def main():
    ...
    parser.add_argument(
        "--count", type=int, default=10, help="Number of simulated customers"
    )
    parser.add_argument(
        "--batch-size", type=int, default=10, help="Concurrent LLM calls per batch"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Generate mock responses without calling LLM",
    )
    parser.add_argument(
        "--backend",
        type=str,
    ...
    parser.add_argument(
        "--no-po", dest="po", action="store_false", help="Skip PO analysis step"
    )
    parser.add_argument(
        "--po-analyst",
        type=str,
    ...
    parser.add_argument(
        "--po-model", type=str, default=None, help="LLM model override for analyst"
    )
    parser.add_argument(
        "--po-agentic",
        action="store_true",
    ...
    logger.info(
        f"[{mode_label}] [{args.mode.upper()} MODE] Feature: {feature_path.name} ({len(feature_text)} chars)"
    )
    if args.mode == "survey":
        logger.info(
